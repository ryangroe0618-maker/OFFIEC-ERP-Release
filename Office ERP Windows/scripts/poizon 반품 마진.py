...
SOURCE_INPUT_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQFuLbNobNHN9skvSn0CJemLbJmplfqcUiLYr_KTlChLSijczOy_INmeYmaMtimT3LyY8YT8FfU2ws8/pub?gid=1940282530&single=true&output=csv"
...
OUTPUT_COLUMNS = SOURCE_COLUMNS + ["내역"]
...
def log(message: str):
    ...
    print(f"[POIZON 반품 마진 {elapsed:6.1f}s] {message}", flush=True)
...
def read_source_sheet(csv_text: str) -> pd.DataFrame:
    ...
    if missing_columns:
        raise DataValidationError(f"원본 시트 필수 열이 없습니다: {missing_columns}")
    ...
    if df.empty:
        raise DataValidationError("업로드할 반품 데이터가 없습니다.")

    df["내역"] = "반품"
    df.loc[df["주문번호"].eq(""), "내역"] = "보관 반품"
