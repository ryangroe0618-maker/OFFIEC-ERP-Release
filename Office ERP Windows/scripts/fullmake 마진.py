...
SOURCE_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTpp7qrBbbOLivbO_P7YLpriTw_6XGoqz_l0JIENxnWCfMIwcNo9bffpBBJNtbAsW80XPakS39yrfnD/pub?gid=0&single=true&output=csv"
...
def log(message: str):
    print(f"[FULLMAKE 마진] {message}", flush=True)
...
def read_source_sheet(csv_text: str) -> pd.DataFrame:
    ...
    if missing_columns:
        raise DataValidationError(f"fullmake 취합 시트에 필수 열이 없습니다: {missing_columns}")
    ...
    if df.empty:
        raise DataValidationError("fullmake 취합 시트에 업로드할 데이터가 없습니다.")
