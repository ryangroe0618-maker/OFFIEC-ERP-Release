...
ACTUAL_SHIPMENT_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfBvby00YLSVYN-dPya7lNeGxtvDHfDDFiW0FwrhW3dHpIIYvupt2yW-t-QNZQhlRjJ98dHIWdNaMC/pub?gid=798336653&single=true&output=csv"
OUT_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSfBvby00YLSVYN-dPya7lNeGxtvDHfDDFiW0FwrhW3dHpIIYvupt2yW-t-QNZQhlRjJ98dHIWdNaMC/pub?gid=594145141&single=true&output=csv"
...
BRANDER_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbwkEjqAetHvOLxoZgWlxL0EjWPbuFL1wGCCd5Pv_Gi5_g5HV5Yjjz7hojXr61ghRRrg-Q/exec"
BRANDER_SPREADSHEET_ID = "1MCYrN0nsrXk5_VbXmp4XQMBPJujDwBQCAMpvALIRa6g"
BRANDER_SHEET_NAME = "브랜더"
...
def log(message: str):
    print(f"[BRAND 마진 수정본] {message}", flush=True)
...
def read_actual_shipment(csv_text: str) -> pd.DataFrame:
    ...
    if not header_positions:
        raise DataValidationError("brand 실출고 시트에서 가로 취합 머리글(품번/컬러/사이즈/수량/매장명)을 찾을 수 없습니다.")
    ...
    if not rows:
        raise DataValidationError("brand 실출고 시트에 가로 취합 데이터가 없습니다.")
...
def build_brander_upload_df(df: pd.DataFrame) -> pd.DataFrame:
    filtered_df = df.loc[df["내역"].map(clean_text).eq("출고")].copy()
    for col in BRANDER_OUTPUT_COLUMNS:
        if col not in filtered_df.columns:
            filtered_df[col] = ""
    return filtered_df[BRANDER_OUTPUT_COLUMNS]
...
def main():
    ...
    brander_df = build_brander_upload_df(result_df)
    upload_to_google_sheet(
        brander_df,
        web_app_url=BRANDER_WEB_APP_URL,
        spreadsheet_id=BRANDER_SPREADSHEET_ID,
        sheet_name=BRANDER_SHEET_NAME,
        columns=BRANDER_OUTPUT_COLUMNS,
    )
