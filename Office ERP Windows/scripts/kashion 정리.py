...
SOURCE_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTU9VO_90AH04WGk0CpddbLa5JHYZg4kiiG0UaHWMaGw82-hVcf5s-XcJtSGeErqMJYP22uy5jqdQ7a/pub?gid=429670088&single=true&output=csv"
STORE_URLS = {
    "스퀘어원": [
...
SOURCE_SLICE_END = 19
SOURCE_EXCLUDE_COLUMN_INDICES = {1}
...
def log(message: str):
    print(f"[KASHION 정리] {message}", flush=True)
...
def summarize_platform_name(order_no: str) -> str:
    ...
    if order_key.startswith("LP"):
        return "KASHION (TM)"
    if order_key.startswith("JD"):
        return "KASHION (JD)"
    return "KASHION"


def is_summary_included(carrier_name: str) -> bool:
    return clean_text(carrier_name) not in {"取消件", "调货中"}
...
def build_platform_summary_df(df: pd.DataFrame) -> pd.DataFrame:
    required_columns = ["날짜", "주문번호", "수량", "판매가", "수수료", "수입", "환율", "KRW", "공급가", "마진", "택배사"]
    ...
    summary_base_df["판매가_num"] = to_number_series(summary_base_df["판매가"])
    summary_base_df["수수료_num"] = to_number_series(summary_base_df["수수료"])
    summary_base_df["수입_num"] = to_number_series(summary_base_df["수입"])
    summary_base_df["환율_num"] = to_number_series(summary_base_df["환율"])
    summary_base_df["KRW_num"] = to_number_series(summary_base_df["KRW"])
    summary_base_df["공급가_num"] = to_number_series(summary_base_df["공급가"])
...
def build_brand_summary_df(df: pd.DataFrame) -> pd.DataFrame:
    required_columns = ["날짜", "주문번호", "브랜드", "수량", "판매가", "환율", "KRW", "공급가", "마진", "택배사"]
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
...
def main():
    ...
    if brand_summary_df.empty:
        log("브랜드 마감 업로드 대상 없음 -> 取消件, 调货中 제외 후 데이터가 없습니다.")
    else:
