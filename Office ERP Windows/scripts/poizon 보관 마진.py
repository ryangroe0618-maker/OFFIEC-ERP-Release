...
SOURCE_INPUT_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vS64g4a29JXVwLOYSLZXRgXJk2NYUl7U0rFnYm3HWlh50KzVC21jttJypukcOVJqhTbDEx_2CSuCRDr/pub?gid=0&single=true&output=csv"
...
def load_poizon_storage_upload_module():
    module_path = Path(__file__).with_name("poizon 보관 정리.py")
    spec = importlib.util.spec_from_file_location("poizon_storage_upload", module_path)
    if spec is None or spec.loader is None:
        raise DataValidationError("poizon 보관 정리.py 모듈을 불러올 수 없습니다.")
...
def add_status_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    store_series = output_df.iloc[:, 9].apply(clean_text)
    status_series = store_series.map(
        lambda store_name: (
            "취소건" if store_name in {"취소", "취소건"}
            else "재고 없음" if store_name in {"재고없음", "재고 없음"}
            else "출고"
        )
    )

    output_df["내역"] = status_series
    return output_df


def build_platform_summary_df(df: pd.DataFrame) -> pd.DataFrame:
    required_columns = ["날짜", "플랫폼", "총 판매가", "총 수수료", "환율", "총 KRW", "총 공급가", "마진", "내역"]
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        raise DataValidationError(f"플랫폼 마감 집계용 필수 열이 없습니다: {missing_columns}")

    shipped_df = df[df["내역"].apply(clean_text).eq("출고")].copy()
    if shipped_df.empty:
...
def build_brand_summary_df(df: pd.DataFrame) -> pd.DataFrame:
    required_columns = ["날짜", "플랫폼", "브랜드", "수량", "총 판매가", "환율", "총 KRW", "총 공급가", "마진", "내역"]
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
    ...
    shipped_df["브랜드"] = shipped_df["브랜드"].apply(clean_text)
    shipped_df["수량_num"] = to_number_series(shipped_df["수량"])
    shipped_df["총 판매가_num"] = to_number_series(shipped_df["총 판매가"])
    shipped_df["환율_num"] = to_number_series(shipped_df["환율"])
    shipped_df["총 KRW_num"] = to_number_series(shipped_df["총 KRW"])
...
def main():
    ...
    upload_module = load_poizon_storage_upload_module()
    csv_texts = upload_module.download_all_csvs()
    source_raw_df = upload_module.read_csv_text(csv_texts["source"])
    size_map_df = upload_module.read_csv_text(csv_texts["online_size"])
    stock_df = upload_module.read_csv_text(csv_texts["stock_prep"])
    exchange_df = upload_module.read_csv_text(csv_texts["exchange"])

    source_df = upload_module.build_output_df(source_raw_df, size_map_df, stock_df, exchange_df)
    source_df["내역"] = "출고"
