...
SOURCE_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTZ-xM3cvMwsZfOXG8CAJC0oGCXJ4bXGcN8_edDCotqk_7cwrRN362ZuzdP7q_JRacgaSXfPzQodHCa/pub?gid=2078527827&single=true&output=csv"
...
WEB_APP_URL = "https://script.google.com/macros/s/AKfycbzD3AH9KMnmBf5hSAnO3XF3PXLlqKj38-vQvvu_E8ZLvLGjlhJN9APRtGm-BeKHwO-F/exec"
SPREADSHEET_ID = "1bxH_7ZMusF6InrPaZLkq4DrQfMPzoDuOGwp88L0df1k"
TARGET_SHEET_NAME = "FULLMAKE"
APPEND_MODE = False
...
def log(message: str):
    print(f"[FULLMAKE 정리] {message}", flush=True)
...
def read_source_sheet(csv_text: str) -> pd.DataFrame:
    ...
    if df_raw.shape[1] < required_cols:
        raise DataValidationError(
            f"fullmake 출고 내역 열 수가 부족합니다. B,C,E,F,L,M 열이 필요한데 실제 {df_raw.shape[1]}열입니다."
        )
    ...
    if df.empty:
        raise DataValidationError("오늘 날짜 기준 업로드할 fullmake 출고 내역 데이터가 없습니다.")
