...
SOURCE_INPUT_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQSGUP7ZizELC9jJqGdxawP_HRsxo-fUZ_5BgMx8NUaEPTJYhH31iiH2-_NEE4Ff9UW3OATcxopoKEU/pub?gid=226771413&single=true&output=csv"
...
def log(message: str):
    print(f"[KREAM] {message}", flush=True)
...
def read_source_sheet(csv_text: str) -> pd.DataFrame:
    ...
    expected_indexes = [0] + list(range(2, 19))
    if df.shape[1] < 19:
        raise DataValidationError(f"원본 시트 열 수가 부족합니다. A, C:S 기준 최소 19열이 필요한데 실제 {df.shape[1]}열입니다.")

    df = df.iloc[:, expected_indexes].copy()
...
def add_status_column(df: pd.DataFrame) -> pd.DataFrame:
    output_df = df.copy()
    store_series = output_df["매장명"].apply(clean_text)
    output_df["내역"] = store_series.map(
        lambda store_name: (
            "취소건" if store_name in {"취소", "취소건"}
            else "재고없음" if store_name in {"재고없음", "재고 없음"}
            else "출고"
        )
    )
    return output_df
...
def build_platform_summary_df(df: pd.DataFrame) -> pd.DataFrame:
    required_columns = ["날짜", "플랫폼", "주문번호", "수량", "총 판매가", "총 수수료", "총 KRW", "총 공급가", "마진", "내역"]
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        raise DataValidationError(f"플랫폼 마감 집계용 필수 열이 없습니다: {missing_columns}")

    summary_base_df = df[df["내역"].apply(clean_text).eq("출고")].copy()
