...
WEB_APP_URL = "https://script.google.com/macros/s/AKfycbyj2W5OnBL8r0I_PbkQ6DYbKVix8TD2yAF3NxWEiGfx1jjnet8bx05bvwlyTHlMlxYC/exec"
SPREADSHEET_ID = "1l6Gvcpij8UQ6O-iSNTWp3eXalc7LK286xNU0HFVDXfU"
TARGET_SHEET_NAME = "KASHION - 반품"
...
TRACKING_HEADER = "운송장"
COMPANY_HEADER = "업체명"
TARGET_COMPANIES = {"RETURNTM", "RETURNJD"}
ORDER_HEADER = "주문번호"
...
def log(message: str):
    print(f"[KASHION 반품] {message}", flush=True)
...
def build_output_df(csv_text: str) -> pd.DataFrame:
    ...
    tracking_header = clean_text(raw_df.iat[HEADER_ROW_INDEX, TRACKING_COL_INDEX])
    if company_header != COMPANY_HEADER:
        raise DataValidationError(f"E열 3행 머리글이 '{COMPANY_HEADER}'이 아닙니다: '{company_header}'")
    if tracking_header != TRACKING_HEADER:
        raise DataValidationError(f"L열 3행 머리글이 '{TRACKING_HEADER}'이 아닙니다: '{tracking_header}'")
    ...
    tracking_series = tracking_series[company_series.isin(TARGET_COMPANIES) & tracking_series.ne("")]
...
def build_order_lookup(csv_texts: list[str]) -> dict[str, str]:
    ...
    for csv_text in csv_texts:
        ...
        order_series = df.iloc[::-1, ORDER_COL_INDEX].apply(clean_text)
        tracking_series = df.iloc[::-1, RETURN_TRACKING_COL_INDEX].apply(clean_text)
...
def add_order_numbers(output_df: pd.DataFrame, order_lookup: dict[str, str]) -> pd.DataFrame:
    ...
    order_numbers = result_df[TRACKING_HEADER].apply(
        lambda tracking_no: order_lookup.get(clean_text(tracking_no), clean_text(tracking_no))
    )
...
def build_platform_detail_df(order_tracking_df: pd.DataFrame, platform_csv_text: str) -> pd.DataFrame:
    ...
    source_df = source_df.iloc[::-1].drop_duplicates(
        subset=["주문번호"],
        keep="first",
    )
    ...
    for _, row in order_tracking_df.iterrows():
        ...
        if not order_no or order_no in matched_orders:
            continue
        output_row = {column: "" for column in OUTPUT_COLUMNS}
        output_row["날짜"] = today_str()
        output_row["주문번호"] = order_no
        output_row["운송장"] = clean_text(row.get(TRACKING_HEADER, ""))
        output_row["내역"] = "반품"
        rows.append(output_row)
